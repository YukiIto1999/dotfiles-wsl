...
let
  ...
  definitionModule = file: registry: {
    _file = "${self}/${file}";
    dotfiles.health.observations = registry;
  };
  evalRegistryModules =
    modules:
    lib.evalModules {
      modules = [ ../impl/observation-registry.nix ] ++ modules;
    };
  evalRegistry =
    registry: evalRegistryModules [ (definitionModule "workstation/module.nix" registry) ];
  forceableValue =
    value:
  ...
  evaluationSucceeds =
    ...
    let
      attempted = builtins.tryEval (
        builtins.deepSeq (forceableValue evaluation.config.dotfiles.health.observations) true
      );
    in
  ...
  ownerRootFor =
    id:
    let
      owner = lib.head (lib.splitString "/" id);
    in
    {
  ...
  misownedDefinitions =
    ...
    lib.concatMap (
      definition:
      builtins.filter (id: ownerRootFor id != rootOf definition.file) (
        builtins.attrNames definition.value
      )
    ) (repositoryDefinitions options.dotfiles.health.observations.definitionsWithLocations);
  ...
  nestedDefinitionModule = file: {
    _file = "${self}/${file}";
    dotfiles.health.observations."host/sample".failureMessage = "nested fixture failure";
  };
  ownerCorrectEvaluation = evalRegistryModules [ (nestedDefinitionModule "workstation/module.nix") ];
  ownerForeignEvaluation = evalRegistryModules [ (nestedDefinitionModule "platform/mcp/module.nix") ];
  dynamicOwnerKeyEvaluation = evalRegistryModules [
    (definitionModule "secrets/sops/module.nix" {
  ...
  invalidDynamicOwnerKeyEvaluations =
    map
      (
        id:
        evalRegistryModules [
          (definitionModule "secrets/sops/module.nix" { ${id} = fixture.valid."host/roster"; })
        ]
      )
      [
        "sops/"
