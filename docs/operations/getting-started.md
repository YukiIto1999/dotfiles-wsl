...
- NixOS-WSL を用意し、`nixos` ユーザーでログインする。[bootstrap script](../../commands/rebuild/bootstrap/impl/bootstrap.sh) はこのユーザーと `/home/nixos/dotfiles-wsl` を初回構築の固定値として検査する。
...
```bash
cd ~/dotfiles-wsl
sudo bash commands/rebuild/bootstrap/impl/bootstrap.sh
```

[bootstrap script](../../commands/rebuild/bootstrap/impl/bootstrap.sh) は次の順序で初回 generation を用意する。
