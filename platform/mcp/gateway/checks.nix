...
let
  ...
  artifact = hostConfig.dotfiles.managedArtifacts."mcp/gateway/default/config";
  syntheticContract = builtins.fromJSON (builtins.readFile ./fixtures/contract.json);
  expected = syntheticContract // {
    targets = builtins.attrNames fronts;
  };
  gateway = hostConfig.dotfiles.platform.mcp.gateway;
  fronts = hostConfig.dotfiles.platform.mcp.fronts;
  ...
  sourceArtifacts = builtins.filter (entry: entry.source == gateway.source) (
    builtins.attrValues hostConfig.dotfiles.managedArtifacts
  );
  variantGateway = variantConfig.dotfiles.platform.mcp.gateway;
  variantArtifact = variantConfig.dotfiles.managedArtifacts."mcp/gateway/default/config";
...
in
{
  ...
  gateway-front-contract =
    assert fronts != { };
    assert syntheticContract.targets == builtins.attrNames fixtureProbes;
  ...
  gateway-artifact-contract =
    ...
    assert hostOptions.dotfiles.platform.mcp.gateway.id.readOnly;
    assert !(hostOptions.dotfiles.platform.mcp.gateway.port.readOnly or false);
    assert hostOptions.dotfiles.platform.mcp.gateway.url.readOnly;
    assert hostOptions.dotfiles.platform.mcp.gateway.service.readOnly;
    assert hostOptions.dotfiles.platform.mcp.gateway.runtimeDirectory.readOnly;
    assert hostOptions.dotfiles.platform.mcp.gateway.source.readOnly;
    assert hostOptions.dotfiles.platform.mcp.gateway.source.type.name == "path";
    assert hostOptions.dotfiles.platform.mcp.gateway.targets.readOnly;
    ...
    assert
      hostConfig.dotfiles.health.observations."artifacts/mcp/gateway/default/config" == {
        kind = "deployed-path";
    ...
    assert serviceConfig.User == hostConfig.dotfiles.workstation.username;
    assert serviceConfig.Environment == [ "HOME=${hostConfig.dotfiles.workstation.homeDir}" ];
