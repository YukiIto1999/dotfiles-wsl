...
let
  ...
  front = hostConfig.dotfiles.platform.mcp.fronts.sonarqube;
  target = hostConfig.dotfiles.platform.mcp.targets.sonarqube;
  ...
  mkIsolationCase =
    ...
    let
      ...
      isolationPkgs = pkgs // {
        callPackage =
          path: args:
          if path == ../../../../platform/mcp/package/mk-npm.nix then
            spec:
            assert builtins.toJSON spec == builtins.toJSON expectedNpmSpec;
            isolationBackendPackage
          else if path == ../../../../platform/mcp/package/mk-server.nix then
            spec:
      ...
      isolatedTarget = evaluation.config.dotfiles.platform.mcp.targets.sonarqube;
      expectedTarget = {
        provider = "sonarqube";
...
in
{
  sonarqube-front =
    ...
    assert
      hostConfig.dotfiles.capabilities.code-quality.sonarqube.credentials.adminPasswordFile
      == expectedPasswordFile;
