...
| MCP front の executable、server transport、server lifecycle、port、backend 依存を変える | 対応する [`mcp/NAME/module.nix`](../../mcp) の target。port は 8770-8789 から取り、backend unit は `waitUnits` に置く。`stdio` は lifecycle に応じた HTTP adapter を生成し、`streamable-http` は provider の foreground server を直接起動する。front と session policy は [`mcp/module.nix`](../../mcp/module.nix) が導く | `dotfiles-rebuild --plan`、`dotfiles-rebuild` |
