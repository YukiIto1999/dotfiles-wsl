...
let
  accounts = hostConfig.dotfiles.accounts;
  accountArtifact = hostConfig.dotfiles.artifacts."accounts/gh-hosts";
  ...
  noWorkIdentityHome =
    noWorkIdentityConfig.home-manager.users.${noWorkIdentityConfig.dotfiles.host.username};
  identityDestinationType = hostOptions.dotfiles.toolchain.git.identity.destinations.default.type;
in
{
  account-deployment-contract =
    assert accounts != [ ];
    assert variantConfig.dotfiles.accounts == accounts;
    ...
    assert
      ...
      == "${noWorkIdentityConfig.dotfiles.host.homeDir}/${gitIdentity.destinations.default}";
