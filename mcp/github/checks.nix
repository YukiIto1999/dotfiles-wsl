...
let
        ...
        modules = [
          ...
          (
            { lib, ... }:
            {
              ...
              config = {
                dotfiles.accounts = accounts;
                dotfiles.mcp.targets = lib.mkForce candidateTargets;
              };
            }
          )
        ];
  ...
  githubTargetNames = builtins.attrNames (
    lib.filterAttrs (_: target: target.provider == "github") hostConfig.dotfiles.mcp.targets
  );
