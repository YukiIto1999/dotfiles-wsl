...
MCP front が既存の secret file を起動時に読む場合は、その secret の `restartUnits` に front service を加える。所有境界は [AI tooling](../architecture/ai-tooling.md#container-platform) に従う。
...
identity の値は `identity/module.nix` の SOPS template を介して配備する。同 module は `secrets/sops/impl/user-secret-file.nix` を明示的に import し、user 所有と mode `0600` を一つの helper から設定する。生成後の Git 設定を直接編集せず、暗号化済み secret を編集して rebuild する。
...
Crawl4AI の API token は `capabilities/web-content/crawl4ai/backend/module.nix` が宣言し、root 所有の environment file から container へ渡す。同じ unit が user 所有の token file path を型付き contract で公開し、同 Capability の MCP adapter が読む。MCP adapter は SOPS の宣言を直接参照しない。

## SearXNG

SearXNG の server secret は `capabilities/web-discovery/searxng/backend/module.nix` が設定 template に差し込み、root 所有の設定ファイルへ配備する。template の更新は SearXNG container の unit を再起動する。同 Capability の MCP adapter は backend の型付き endpoint だけを読む。
...
SonarQube の database password は `capabilities/code-quality/sonarqube/server/module.nix` と `capabilities/code-quality/sonarqube/database/module.nix` が root 所有の environment file へ展開する。template の更新は対応する container unit を再起動する。

## SonarQube admin password の初回登録

admin password は `capabilities/code-quality/sonarqube/server/module.nix` が宣言し、user 所有の runtime file path を型付き contract で公開する。初回登録では、`capabilities/code-quality/sonarqube/provisioning/module.nix` の service が SonarQube の初期値 `admin` を SOPS の宣言値へ変更する。server が未起動で失敗すると 60 秒後に再試行する。成功後は inactive へ戻り、timer が 1 時間ごとに再実行する。
