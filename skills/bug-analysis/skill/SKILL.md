...
- browserのDOM、console、networkを含む既知の再現経路は`browser-operation`で観測し、その証拠をsignalとして使う
...
performanceだけの劣化はこのSkillの責務外である。脆弱性や攻撃可能性は`security-review`、既存diffの品質判定は`code-review`の責務である。
