...
検証済みまたは技術的に妥当なfindingの修正をユーザーが明示した場合は、finding、壊れたsecurity invariant、再現証拠、攻撃経路を`validated-finding-attack-path`としてimplementerへ引き渡す。implementerが別phaseで`fix-finding`を読み、修正とregression testを所有する。security agentは修正を実装しない。
