...
4. 因果と経緯は地の文で書く。箇条書きは、項目が同じ構造と同じ抽象度で並ぶときだけ使う。
5. 同じ概念には同じ語を使う。対象、主体、条件を曖昧な「これ」「ツール」「仕組み」で隠さない。
6. 読者と媒体に合う敬体か常体を選び、既存文書に合わせる。語尾を機械的に散らさない。同じ文型や同じ節構成が3回続いたら、一つは入り口を変える。均一と反復のどちらを避けるか迷ったら、読者の負荷が下がるほうを選ぶ。
7. [推敲観点](references/revision.md)を使い、内容を変えずに空句、過剰な評価、定型構成、不要な装飾を除く。既存文書では直す箇所を選び、全体へ一律に当てない。
