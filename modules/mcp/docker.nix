...
in
{
  ...
  _module.args.mkMcpBackend = name: { image, imageFile ? null, volumes ? [ ], extraOptions ? [ ], ports ? [ ], deps ? [ ] }: {
    containers."${name}" =
      { inherit image; }
      // lib.optionalAttrs (imageFile != null) { inherit imageFile; }
      // lib.optionalAttrs (volumes != [ ]) { inherit volumes; }
      // {
        extraOptions = [ "--network=mcp-backends" ]
          ++ extraOptions
          ++ lib.concatMap (port: [ "-p" "127.0.0.1:${port}:${port}" ]) ports;
      };
