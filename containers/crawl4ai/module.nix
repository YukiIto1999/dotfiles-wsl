...
in
{
  options.dotfiles.containers.crawl4ai.credentials.apiTokenFile = lib.mkOption {
    type = lib.types.str;
    readOnly = true;
  };
  ...
  config = {
    dotfiles.containers = {
      crawl4ai.credentials.apiTokenFile = config.sops.secrets."crawl4ai/api_token".path;
