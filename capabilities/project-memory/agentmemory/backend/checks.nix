...
let
  ...
  service = hostConfig.dotfiles.platform.containers.services.agentmemory;
  image = service.images.agentmemory;
  container = hostConfig.virtualisation.oci-containers.containers.agentmemory;
  engineConfig = hostConfig.dotfiles.managedArtifacts."containers/agentmemory/config".source;
  ...
  upstreamRoot = toString hostConfig.dotfiles.capabilities.project-memory.agentmemory.upstream.root;
...
in
{
  agentmemory-container =
    assert
      hostConfig.dotfiles.capabilities.project-memory.agentmemory.upstream.version == expectedVersion;
    assert
      service.containerPolicy.secretReaders == {
