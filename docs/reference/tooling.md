...
target は provider、executable、server transport、server lifecycle、port、backend unit、probe を持つ。front は target から一度だけ導かれ、backend unit を `requires` と `after` に持つ。`stdio` transport の `service` lifecycle は stdio server を共有し、`session` lifecycle は downstream session ごとに生成する。`streamable-http` transport は provider の foreground server を直接使う。公開 agentgateway は全 front を一つの URL へ束ねるが、front service の依存は持たない。credential、container、host process の境界は[セキュリティ設計](../architecture/security.md)を参照する。
